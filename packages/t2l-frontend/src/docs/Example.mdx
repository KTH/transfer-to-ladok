...
# Try the Transfer to Ladok API

In this document we will guide you through Transfer to Ladok in **stage**. It means that we will use test environments of Ladok and Canvas:

- Ladok: https://www.integrationstest.ladok.se
- Canvas: https://kth.test.instructure.com

The root URL for Transfer to Ladok API is `https://app-r.referens.sys.kth.se`.
...
Go to https://kth.test.instructure.com/profile/settings to get the Canvas token.
...
```
GET /transfer-to-ladok/api/courses/:courseId/sections
```
...
```
GET /transfer-to-ladok/api/courses/:courseId/ladok-grades?aktivitetstillfalle=<<A>>
GET /transfer-to-ladok/api/courses/:courseId/ladok-grades?kurstillfalle=<B>&utbildningsinstans=<C>
```
...
```
POST /transfer-to-ladok/api/courses/:courseId/ladok-grades
```
...
# Send results to Ladok from a Canvas course room

## 1. Get all course rounds and module in a Canvas course room
...
```json
{
  "kurstillfalle": [
    {
      "id": "<< course round ID >>",
      "utbildningsinstans": "<< ID if you want to chose final grade >>",
      "courseCode": "SF1624",
      "roundCode": "50584",
      "modules": [
        {
          "utbildningsinstans": "<< ID if you want to chose module TEN1 >>",
          "code": "TEN1",
          "name": "Tentamen"
        }
      ]
...
- Chose one of the course rounds (`kurstillfalle`) in the list. Keep its ID
- In the selected course round, choose one of the modules. Keep the `utbildningsinstans` value in that module.

  If you don't want to transfer results to a module but to the final grades of the course, chose the `utbildningsinstans` attribute in the `kurstillfalle` object.

## 2. Get a list of students in the selected `kurstillfalle+utbildningsinstans`

Request:

```
GET /transfer-to-ladok/api/courses/:courseId/ladok-grades?kurstillfalle=<< course round ID >>&utbildningsinstans=<< id for final grade or module >>
```
...
{
  "destination": {
    "kurstillfalle": "<< course round ID from step 1 >>",
    "utbildningsinstans": "<< ID for module or final grade from step 1 >>"
  },
  "results": [
    {
      "id": "<< Student ID >>",
